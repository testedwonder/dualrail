...
PUBLIC_SOURCE_PATTERN = re.compile(
    r"^knowledge/(?:research/public-technical-sources\.md|"
    r"topics/[a-z0-9-]+/references\.md)$"
)
...
def _validate_sources(
    ...
    for document in documents.values():
        ...
        for reference in source_files:
            ...
            if not PUBLIC_SOURCE_PATTERN.fullmatch(reference):
                errors.append(
                    f"{document.relative_path}: source file is not an approved public claim map: {reference}"
                )
                continue
            if not source_path.is_file():
                ...
                continue
            source_document = documents.get(source_path)
            if source_document is None or source_document.metadata.get("kind") != "index":
                errors.append(
                    f"{document.relative_path}: source file is not a knowledge index: {reference}"
                )
            if source_path == document.path:
                errors.append(
                    f"{document.relative_path}: source file cannot reference itself: {reference}"
                )
...
def validate_repository(
    ...
    _validate_sources(documents, repository_root, errors)
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        help="Repository root containing the public knowledge tree.",
    )
    parser.add_argument(
        "--skip-examples",
        action="store_true",
    ...
    if result.errors:
        ...
        print(
            ...
            f"{result.checked_markdown_files} knowledge Markdown file(s)."
        )
        return 1
